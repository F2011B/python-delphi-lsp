...
def test_release_metadata_declares_3_1_1_sole_namespace_author_and_windows_support() -> None:
    pyproject = (ROOT / 'pyproject.toml').read_text(encoding='utf-8')
    project = _section('project', pyproject)
    scripts = _section('project.scripts', pyproject)

    assert 'version = "3.1.1"' in project
...
def test_readme_documents_v3_release_plugin_protocol_discovery_and_vllm_proof() -> None:
    ...
    assert 'Version 3.1.1' in readme
...
def test_release_3_1_1_notes_document_verified_remediation() -> None:
    notes = (ROOT / 'docs' / 'release-3.1.1.md').read_text(encoding='utf-8')

    assert '# Python Delphi LSP 3.1.1' in notes
    assert '38 verified defects' in notes
    assert '861 passed' in notes
